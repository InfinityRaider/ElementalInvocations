...
public class DamageEffect implements ISpellEffect {
	...
	public boolean apply(EntityPlayer caster, IPotencyMap potencies, int channelTick) {
		...
		AxisAlignedBB area = AreaHelper.getArea(caster.getPositionVector(), potencies.getPotency(Element.DEATH));
		List<Entity> ents = caster.getEntityWorld().getEntitiesWithinAABB(Entity.class, area);
		ents.forEach(e -> e.attackEntityFrom(DamageSource.magic, potencies.getPotency(Element.DEATH)));
		return false;
	}
